    ...
    fn write_fprs(&mut self) -> Result<&str, &str> {
        ...
        let result = unsafe {
            ptrace(
                ...
                fprs as *const _ as *const std::ffi::c_void, // we were giving a pointer to the
                                                             // stack location holding the
                                                             // reference of the data not the data
                                                             // itself
            )
        };
        if result < 0 {
            ...
            return Err("Couldn't write to user");
        }
        Ok("Register Updated")
    }
